	...
	def validate(self):
		...
		def gen_update_card_series(state,city,centre,card_number):
			...
			if card_number:
				...
				card_check = frappe.db.sql("""select name from `tabDignity Senior Citizen Marathi` where card_number = %s""",card_number)
				if card_check:
					name = card_check[0][0]
					if self.name == name:
						pass
...
@frappe.whitelist()
def get_card_series_marathi(state,city,centre):
	conditions=""
