...
frappe.ui.form.on("Dignity Senior Citizen", {
	...
	tahsildar_name: function(frm, cdt, cdn) {
		var senior_citizen = frappe.model.get_doc(cdt, cdn);
		if (senior_citizen.tahsildar_centre)
		{			frappe.call({
				method: "dignity.dignity_foundation.doctype.dignity_senior_citizen.dignity_senior_citizen.get_tahsildar_series",
				args: {
					state: frm.doc.tahsildar_state,
					city: frm.doc.tahsildar_city,
					tahsildar_name: frm.doc.tahsildar_name
				},
				callback: function(r) {
					frappe.model.set_value(cdt, cdn, "new_tahsildar_series", r.message[0]);
					frappe.model.set_value(cdt, cdn, "tahsildar_no", r.message[1]);
				}
			});
		}
 	},
